 ...
 * Generated: true
 **/
public class AiTelemetryPlatformGenPage extends AiTelemetryPlatformGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListAiTelemetryPlatform_(Wrap<SearchList<AiTelemetryPlatform>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListAiTelemetryPlatform_ != null)
      w.o(Optional.ofNullable(searchListAiTelemetryPlatform_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListAiTelemetryPlatform_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListAiTelemetryPlatform_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListAiTelemetryPlatform_.getStart().longValue();
    Long rows = searchListAiTelemetryPlatform_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListAiTelemetryPlatform_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListAiTelemetryPlatform_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    AiTelemetryPlatform.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(AiTelemetryPlatform.displayNameAiTelemetryPlatform(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(AiTelemetryPlatform.classSimpleNameAiTelemetryPlatform(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListAiTelemetryPlatform_.getRequest().getQuery()).filter(fq -> fq.startsWith(AiTelemetryPlatform.varIndexedAiTelemetryPlatform(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : AiTelemetryPlatform.varsFqForClass()) {
      String varIndexed = AiTelemetryPlatform.varIndexedAiTelemetryPlatform(var);
      String varStored = AiTelemetryPlatform.varStoredAiTelemetryPlatform(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(AiTelemetryPlatform.displayNameAiTelemetryPlatform(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(AiTelemetryPlatform.classSimpleNameAiTelemetryPlatform(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListAiTelemetryPlatform_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(AiTelemetryPlatform.varIndexedAiTelemetryPlatform(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = AiTelemetryPlatform.searchVarAiTelemetryPlatform(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    AiTelemetryPlatform.varsRangeForClass().forEach(var -> {
      String varIndexed = AiTelemetryPlatform.varIndexedAiTelemetryPlatform(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(AiTelemetryPlatform.displayNameAiTelemetryPlatform(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(AiTelemetryPlatform.classSimpleNameAiTelemetryPlatform(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListAiTelemetryPlatform_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(AiTelemetryPlatform.varIndexedAiTelemetryPlatform(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListAiTelemetryPlatform_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListAiTelemetryPlatform_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListAiTelemetryPlatform_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListAiTelemetryPlatform_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListAiTelemetryPlatform_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = AiTelemetryPlatform.searchVarAiTelemetryPlatform(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", AiTelemetryPlatform.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListAiTelemetryPlatform_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = AiTelemetryPlatform.searchVarAiTelemetryPlatform(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", AiTelemetryPlatform.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListAiTelemetryPlatform_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListAiTelemetryPlatform_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListAiTelemetryPlatform_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListAiTelemetryPlatform_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return AiTelemetryPlatform.searchVarAiTelemetryPlatform(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListAiTelemetryPlatform_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListAiTelemetryPlatform_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListAiTelemetryPlatform_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListAiTelemetryPlatform_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListAiTelemetryPlatform_.getDefaultSort()) {
      Optional.ofNullable(searchListAiTelemetryPlatform_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = AiTelemetryPlatform.searchVarAiTelemetryPlatform(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListAiTelemetryPlatform_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = AiTelemetryPlatform.searchVarAiTelemetryPlatform(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListAiTelemetryPlatform_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListAiTelemetryPlatform_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listAiTelemetryPlatform(JsonArray l) {
    Optional.ofNullable(searchListAiTelemetryPlatform_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListAiTelemetryPlatform_ == null ? 0 : searchListAiTelemetryPlatform_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<AiTelemetryPlatform> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("pageId")).orElse(null) != null)
      w.o(searchListAiTelemetryPlatform_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    ...
    else if(searchListAiTelemetryPlatform_ == null || resultCount == 0)
  ...
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/ai-telemetry-platform");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/ai-telemetry-platform");
  }
  ...
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/ai-telemetry-platform-999.png");
  }
  ...
  protected void _pageUriAiTelemetryPlatform(Wrap<String> c) {
      c.o("/en-us/search/ai-telemetry-platform");
  }
}
