 ...
 * Generated: true
 **/
public class JobTemplateGenPage extends JobTemplateGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListJobTemplate_(Wrap<SearchList<JobTemplate>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListJobTemplate_ != null)
      w.o(Optional.ofNullable(searchListJobTemplate_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListJobTemplate_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListJobTemplate_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListJobTemplate_.getStart().longValue();
    Long rows = searchListJobTemplate_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListJobTemplate_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListJobTemplate_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    JobTemplate.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(JobTemplate.displayNameJobTemplate(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(JobTemplate.classSimpleNameJobTemplate(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListJobTemplate_.getRequest().getQuery()).filter(fq -> fq.startsWith(JobTemplate.varIndexedJobTemplate(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : JobTemplate.varsFqForClass()) {
      String varIndexed = JobTemplate.varIndexedJobTemplate(var);
      String varStored = JobTemplate.varStoredJobTemplate(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(JobTemplate.displayNameJobTemplate(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(JobTemplate.classSimpleNameJobTemplate(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListJobTemplate_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(JobTemplate.varIndexedJobTemplate(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = JobTemplate.searchVarJobTemplate(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    JobTemplate.varsRangeForClass().forEach(var -> {
      String varIndexed = JobTemplate.varIndexedJobTemplate(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(JobTemplate.displayNameJobTemplate(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(JobTemplate.classSimpleNameJobTemplate(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListJobTemplate_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(JobTemplate.varIndexedJobTemplate(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListJobTemplate_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListJobTemplate_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListJobTemplate_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListJobTemplate_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListJobTemplate_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = JobTemplate.searchVarJobTemplate(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", JobTemplate.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListJobTemplate_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = JobTemplate.searchVarJobTemplate(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", JobTemplate.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListJobTemplate_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListJobTemplate_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListJobTemplate_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListJobTemplate_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return JobTemplate.searchVarJobTemplate(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListJobTemplate_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListJobTemplate_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListJobTemplate_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListJobTemplate_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListJobTemplate_.getDefaultSort()) {
      Optional.ofNullable(searchListJobTemplate_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = JobTemplate.searchVarJobTemplate(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListJobTemplate_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = JobTemplate.searchVarJobTemplate(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListJobTemplate_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListJobTemplate_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listJobTemplate(JsonArray l) {
    Optional.ofNullable(searchListJobTemplate_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListJobTemplate_ == null ? 0 : searchListJobTemplate_.size());
  }
  ...
  **/
  protected void _result(Wrap<JobTemplate> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("jobTemplateResource")).orElse(null) != null)
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("job templates");
    else if(searchListJobTemplate_ == null || resultCount == 0)
  ...
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/job-template");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/job-template");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("A job template to be run on a computer in Ansible Automation Platform. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/job-template-999.png");
  }
  ...
  protected void _pageUriJobTemplate(Wrap<String> c) {
      c.o("/en-us/search/job-template");
  }
}
