 ...
 * Generated: true
 **/
public class ProviderIntentGenPage extends ProviderIntentGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListProviderIntent_(Wrap<SearchList<ProviderIntent>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListProviderIntent_ != null)
      w.o(Optional.ofNullable(searchListProviderIntent_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListProviderIntent_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListProviderIntent_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListProviderIntent_.getStart().longValue();
    Long rows = searchListProviderIntent_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListProviderIntent_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListProviderIntent_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    ProviderIntent.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(ProviderIntent.displayNameProviderIntent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(ProviderIntent.classSimpleNameProviderIntent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListProviderIntent_.getRequest().getQuery()).filter(fq -> fq.startsWith(ProviderIntent.varIndexedProviderIntent(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : ProviderIntent.varsFqForClass()) {
      String varIndexed = ProviderIntent.varIndexedProviderIntent(var);
      String varStored = ProviderIntent.varStoredProviderIntent(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(ProviderIntent.displayNameProviderIntent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(ProviderIntent.classSimpleNameProviderIntent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListProviderIntent_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(ProviderIntent.varIndexedProviderIntent(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = ProviderIntent.searchVarProviderIntent(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    ProviderIntent.varsRangeForClass().forEach(var -> {
      String varIndexed = ProviderIntent.varIndexedProviderIntent(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(ProviderIntent.displayNameProviderIntent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(ProviderIntent.classSimpleNameProviderIntent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListProviderIntent_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(ProviderIntent.varIndexedProviderIntent(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListProviderIntent_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListProviderIntent_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListProviderIntent_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListProviderIntent_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListProviderIntent_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = ProviderIntent.searchVarProviderIntent(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", ProviderIntent.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListProviderIntent_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = ProviderIntent.searchVarProviderIntent(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", ProviderIntent.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListProviderIntent_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListProviderIntent_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListProviderIntent_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListProviderIntent_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return ProviderIntent.searchVarProviderIntent(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListProviderIntent_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListProviderIntent_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListProviderIntent_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListProviderIntent_.getFacetPivotMinCount()).orElse(0));
  }

  @Override
  protected void _defaultSortVars(List<String> l) {
    if(!searchListProviderIntent_.getDefaultSort()) {
      Optional.ofNullable(searchListProviderIntent_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = ProviderIntent.searchVarProviderIntent(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListProviderIntent_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = ProviderIntent.searchVarProviderIntent(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListProviderIntent_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListProviderIntent_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listProviderIntent(JsonArray l) {
    Optional.ofNullable(searchListProviderIntent_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListProviderIntent_ == null ? 0 : searchListProviderIntent_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<ProviderIntent> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("providerResource")).orElse(null) != null)
      w.o(searchListProviderIntent_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("provider intents");
    else if(searchListProviderIntent_ == null || resultCount == 0)
      c.o("no provider intent found");
    else
      c.o("provider intents");
  }
  ...
  protected void _classAllName(Wrap<String> w) {
    w.o("all provider intents");
  }

  @Override
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/intent/provider");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/intent/provider");
  }
  ...
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/intent/provider-999.png");
  }
  ...
  protected void _pageUriProviderIntent(Wrap<String> c) {
      c.o("/en-us/search/intent/provider");
  }
}
