...
public class HostEnUSGenApiServiceImpl extends BaseApiServiceImpl implements HostEnUSGenApiService {
  ...
  public void searchHost(ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
    ...
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
      try {
        ...
        groups.stream().map(group -> {
              Matcher mPermission = Pattern.compile("^/(.*-?HOST-([a-z0-9\\-]+))-(\\w+)$").matcher(group);
              return mPermission.find() ? mPermission : null;
            }).filter(v -> v != null).forEach(mPermission -> {
              form.add("permission", String.format("%s#%s", mPermission.group(1), mPermission.group(3)));
            });
      ...
      for(String entityVar : methodNames) {
        switch(entityVar) {
          ...
          case "setTenantId":
              ...
              if(bParams.size() > 0)
                bSql.append(", ");
              bSql.append(Host.VAR_tenantId + "=$" + num);
              num++;
  ...
  public void templateEditPageHost(JsonObject ctx, HostPage page, SearchList<Host> listHost, Promise<String> promise) {
    try {
      ...
      Path resourceTemplatePath = Path.of(siteTemplatePath, pageTemplateUri);
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "en-us/edit/host/HostEditPage.htm"), Charset.forName("UTF-8"));
        String renderedTemplate = jinjava.render(template, ctx.getMap());
        promise.complete(renderedTemplate);
      } else if(pageTemplateUri.endsWith(".md")) {
        String template = siteTemplatePath == null ? Resources.toString(Resources.getResource(resourceTemplatePath.toString()), StandardCharsets.UTF_8) : Files.readString(resourceTemplatePath, Charset.forName("UTF-8"));
  ...
  public Future<JsonObject> upsertHost(Host o, Boolean inheritPrimaryKey, Boolean patch) {
            ...
            if(oTenant == null) {
              ...
              HostInventory.fqHostInventory(siteRequest, HostInventory.VAR_inventoryResource, inventoryResource).onSuccess(oHostInventory -> {
                try {
                  ...
                  LOG.error(String.format("upsertHost failed. "), ex);
                  promise.tryFail(ex);
                }
              }).onFailure(ex -> {
                promise.fail(ex);
  ...
  public Future<Void> persistHost(Host o, Boolean patch) {
    ...
    try {
      ...
      SqlConnection sqlConnection = siteRequest.getSqlConnection();
      Long pk = o.getPk();
      sqlConnection.preparedQuery("SELECT tenantResource, tenantId, created, aapOrganizationId, inventoryResource, archived, aapHostId, hostName, ipAddress, hostId, sessionId, hostResource, userKey, hostDescription, aapInventoryId, inventoryName, objectTitle, eventSubscriptions, displayPage, tags, editPage, userPage, download FROM Host WHERE pk=$1")
          .collecting(Collectors.toList())
          .execute(Tuple.of(pk)
          ).onSuccess(result -> {
        try {
  ...
  public Future<JsonObject> generatePageBody(ComputateSiteRequest siteRequest, Map<String, Object> ctx, String templatePath, String classSimpleName, String pageTemplate) {
    ...
    try {
      ...
      o.persistForClass(Host.VAR_tenantId, Host.staticSetTenantId(siteRequest2, (String)result.get(Host.VAR_tenantId)));
      ...
      o.persistForClass(Host.VAR_tags, Host.staticSetTags(siteRequest2, (String)result.get(Host.VAR_tags)));
