 ...
 * Generated: true
 **/
public class HostCheckCRGenPage extends HostCheckCRGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListHostCheckCR_(Wrap<SearchList<HostCheckCR>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListHostCheckCR_ != null)
      w.o(Optional.ofNullable(searchListHostCheckCR_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListHostCheckCR_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListHostCheckCR_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListHostCheckCR_.getStart().longValue();
    Long rows = searchListHostCheckCR_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListHostCheckCR_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListHostCheckCR_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    HostCheckCR.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(HostCheckCR.displayNameHostCheckCR(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(HostCheckCR.classSimpleNameHostCheckCR(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListHostCheckCR_.getRequest().getQuery()).filter(fq -> fq.startsWith(HostCheckCR.varIndexedHostCheckCR(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : HostCheckCR.varsFqForClass()) {
      String varIndexed = HostCheckCR.varIndexedHostCheckCR(var);
      String varStored = HostCheckCR.varStoredHostCheckCR(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(HostCheckCR.displayNameHostCheckCR(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(HostCheckCR.classSimpleNameHostCheckCR(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListHostCheckCR_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(HostCheckCR.varIndexedHostCheckCR(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = HostCheckCR.searchVarHostCheckCR(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    HostCheckCR.varsRangeForClass().forEach(var -> {
      String varIndexed = HostCheckCR.varIndexedHostCheckCR(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(HostCheckCR.displayNameHostCheckCR(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(HostCheckCR.classSimpleNameHostCheckCR(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListHostCheckCR_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(HostCheckCR.varIndexedHostCheckCR(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListHostCheckCR_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListHostCheckCR_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListHostCheckCR_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListHostCheckCR_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListHostCheckCR_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = HostCheckCR.searchVarHostCheckCR(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", HostCheckCR.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListHostCheckCR_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = HostCheckCR.searchVarHostCheckCR(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", HostCheckCR.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListHostCheckCR_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListHostCheckCR_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListHostCheckCR_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListHostCheckCR_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return HostCheckCR.searchVarHostCheckCR(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListHostCheckCR_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListHostCheckCR_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListHostCheckCR_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListHostCheckCR_.getFacetPivotMinCount()).orElse(0));
  }

  @Override
  protected void _defaultSortVars(List<String> l) {
    if(!searchListHostCheckCR_.getDefaultSort()) {
      Optional.ofNullable(searchListHostCheckCR_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = HostCheckCR.searchVarHostCheckCR(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListHostCheckCR_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = HostCheckCR.searchVarHostCheckCR(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListHostCheckCR_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListHostCheckCR_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listHostCheckCR(JsonArray l) {
    Optional.ofNullable(searchListHostCheckCR_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListHostCheckCR_ == null ? 0 : searchListHostCheckCR_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<HostCheckCR> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("changeRequestId")).orElse(null) != null)
      w.o(searchListHostCheckCR_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("host check change requests");
    else if(searchListHostCheckCR_ == null || resultCount == 0)
      c.o("no host check change request found");
    else
      c.o("host check change requests");
  }
  ...
  protected void _classAllName(Wrap<String> w) {
    w.o("all host check change requests");
  }

  @Override
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/host-check-cr");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/host-check-cr");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("A new or updated check to be performed on a computer. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/host-check-cr-999.png");
  }
  ...
  protected void _pageUriHostCheckCR(Wrap<String> c) {
      c.o("/en-us/search/host-check-cr");
  }
}
