 ...
 * Generated: true
 **/
public class RequestApprovalGenPage extends RequestApprovalGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListRequestApproval_(Wrap<SearchList<RequestApproval>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListRequestApproval_ != null)
      w.o(Optional.ofNullable(searchListRequestApproval_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListRequestApproval_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListRequestApproval_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListRequestApproval_.getStart().longValue();
    Long rows = searchListRequestApproval_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListRequestApproval_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListRequestApproval_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    RequestApproval.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(RequestApproval.displayNameRequestApproval(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(RequestApproval.classSimpleNameRequestApproval(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListRequestApproval_.getRequest().getQuery()).filter(fq -> fq.startsWith(RequestApproval.varIndexedRequestApproval(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : RequestApproval.varsFqForClass()) {
      String varIndexed = RequestApproval.varIndexedRequestApproval(var);
      String varStored = RequestApproval.varStoredRequestApproval(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(RequestApproval.displayNameRequestApproval(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(RequestApproval.classSimpleNameRequestApproval(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListRequestApproval_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(RequestApproval.varIndexedRequestApproval(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = RequestApproval.searchVarRequestApproval(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    RequestApproval.varsRangeForClass().forEach(var -> {
      String varIndexed = RequestApproval.varIndexedRequestApproval(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(RequestApproval.displayNameRequestApproval(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(RequestApproval.classSimpleNameRequestApproval(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListRequestApproval_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(RequestApproval.varIndexedRequestApproval(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListRequestApproval_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListRequestApproval_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListRequestApproval_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListRequestApproval_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListRequestApproval_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = RequestApproval.searchVarRequestApproval(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", RequestApproval.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListRequestApproval_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = RequestApproval.searchVarRequestApproval(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", RequestApproval.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListRequestApproval_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListRequestApproval_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListRequestApproval_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListRequestApproval_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return RequestApproval.searchVarRequestApproval(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListRequestApproval_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListRequestApproval_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListRequestApproval_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListRequestApproval_.getFacetPivotMinCount()).orElse(0));
  }

  @Override
  protected void _defaultSortVars(List<String> l) {
    if(!searchListRequestApproval_.getDefaultSort()) {
      Optional.ofNullable(searchListRequestApproval_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = RequestApproval.searchVarRequestApproval(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListRequestApproval_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = RequestApproval.searchVarRequestApproval(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListRequestApproval_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListRequestApproval_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listRequestApproval(JsonArray l) {
    Optional.ofNullable(searchListRequestApproval_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListRequestApproval_ == null ? 0 : searchListRequestApproval_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<RequestApproval> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("approvalId")).orElse(null) != null)
      w.o(searchListRequestApproval_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getApprovalTitle() != null)
      ...
      c.o("request approvals");
    else if(searchListRequestApproval_ == null || resultCount == 0)
      c.o("no request approval found");
    else
      c.o("request approvals");
  }
  ...
  protected void _classAllName(Wrap<String> w) {
    w.o("all request approvals");
  }

  @Override
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/approval");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/approval");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("Individual approvals per request and per approver. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/approval-999.png");
  }

  @Override
  protected void _classIcon(Wrap<String> c) {
      c.o("<i class=\"{{ FONTAWESOME_STYLE }} fa-thumbs-up\"></i>");
  }

  protected void _pageUriRequestApproval(Wrap<String> c) {
      c.o("/en-us/search/approval");
  }
}
