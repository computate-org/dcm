 ...
 * Generated: true
 **/
public class TenantProviderGenPage extends TenantProviderGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListTenantProvider_(Wrap<SearchList<TenantProvider>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListTenantProvider_ != null)
      w.o(Optional.ofNullable(searchListTenantProvider_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListTenantProvider_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListTenantProvider_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListTenantProvider_.getStart().longValue();
    Long rows = searchListTenantProvider_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListTenantProvider_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListTenantProvider_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    TenantProvider.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(TenantProvider.displayNameTenantProvider(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(TenantProvider.classSimpleNameTenantProvider(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListTenantProvider_.getRequest().getQuery()).filter(fq -> fq.startsWith(TenantProvider.varIndexedTenantProvider(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : TenantProvider.varsFqForClass()) {
      String varIndexed = TenantProvider.varIndexedTenantProvider(var);
      String varStored = TenantProvider.varStoredTenantProvider(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(TenantProvider.displayNameTenantProvider(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(TenantProvider.classSimpleNameTenantProvider(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListTenantProvider_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TenantProvider.varIndexedTenantProvider(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = TenantProvider.searchVarTenantProvider(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    TenantProvider.varsRangeForClass().forEach(var -> {
      String varIndexed = TenantProvider.varIndexedTenantProvider(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(TenantProvider.displayNameTenantProvider(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(TenantProvider.classSimpleNameTenantProvider(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListTenantProvider_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TenantProvider.varIndexedTenantProvider(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListTenantProvider_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListTenantProvider_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListTenantProvider_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListTenantProvider_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListTenantProvider_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = TenantProvider.searchVarTenantProvider(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", TenantProvider.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListTenantProvider_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = TenantProvider.searchVarTenantProvider(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", TenantProvider.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListTenantProvider_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListTenantProvider_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListTenantProvider_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListTenantProvider_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return TenantProvider.searchVarTenantProvider(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListTenantProvider_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListTenantProvider_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListTenantProvider_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListTenantProvider_.getFacetPivotMinCount()).orElse(0));
  }

  @Override
  protected void _defaultSortVars(List<String> l) {
    if(!searchListTenantProvider_.getDefaultSort()) {
      Optional.ofNullable(searchListTenantProvider_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = TenantProvider.searchVarTenantProvider(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListTenantProvider_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = TenantProvider.searchVarTenantProvider(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListTenantProvider_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListTenantProvider_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listTenantProvider(JsonArray l) {
    Optional.ofNullable(searchListTenantProvider_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListTenantProvider_ == null ? 0 : searchListTenantProvider_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<TenantProvider> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("providerId")).orElse(null) != null)
      w.o(searchListTenantProvider_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("tenant intents");
    else if(searchListTenantProvider_ == null || resultCount == 0)
      c.o("no tenant intent found");
    else
      c.o("tenant intents");
  }
  ...
  protected void _classAllName(Wrap<String> w) {
    w.o("all tenant intents");
  }

  @Override
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/provider/tenant");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/provider/tenant");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("A provider for requesting a TenantIntent to be realized. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/provider/tenant-999.png");
  }
  ...
  protected void _pageUriTenantProvider(Wrap<String> c) {
      c.o("/en-us/search/provider/tenant");
  }
}
