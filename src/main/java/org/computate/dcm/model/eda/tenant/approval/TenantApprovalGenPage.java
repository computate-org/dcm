 ...
 * Generated: true
 **/
public class TenantApprovalGenPage extends TenantApprovalGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListTenantApproval_(Wrap<SearchList<TenantApproval>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListTenantApproval_ != null)
      w.o(Optional.ofNullable(searchListTenantApproval_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListTenantApproval_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListTenantApproval_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListTenantApproval_.getStart().longValue();
    Long rows = searchListTenantApproval_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListTenantApproval_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListTenantApproval_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    TenantApproval.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(TenantApproval.displayNameTenantApproval(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(TenantApproval.classSimpleNameTenantApproval(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListTenantApproval_.getRequest().getQuery()).filter(fq -> fq.startsWith(TenantApproval.varIndexedTenantApproval(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : TenantApproval.varsFqForClass()) {
      String varIndexed = TenantApproval.varIndexedTenantApproval(var);
      String varStored = TenantApproval.varStoredTenantApproval(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(TenantApproval.displayNameTenantApproval(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(TenantApproval.classSimpleNameTenantApproval(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListTenantApproval_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TenantApproval.varIndexedTenantApproval(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = TenantApproval.searchVarTenantApproval(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    TenantApproval.varsRangeForClass().forEach(var -> {
      String varIndexed = TenantApproval.varIndexedTenantApproval(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(TenantApproval.displayNameTenantApproval(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(TenantApproval.classSimpleNameTenantApproval(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListTenantApproval_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TenantApproval.varIndexedTenantApproval(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListTenantApproval_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListTenantApproval_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListTenantApproval_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListTenantApproval_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListTenantApproval_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = TenantApproval.searchVarTenantApproval(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", TenantApproval.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListTenantApproval_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = TenantApproval.searchVarTenantApproval(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", TenantApproval.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListTenantApproval_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListTenantApproval_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListTenantApproval_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListTenantApproval_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return TenantApproval.searchVarTenantApproval(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListTenantApproval_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListTenantApproval_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListTenantApproval_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListTenantApproval_.getFacetPivotMinCount()).orElse(0));
  }

  @Override
  protected void _defaultSortVars(List<String> l) {
    if(!searchListTenantApproval_.getDefaultSort()) {
      Optional.ofNullable(searchListTenantApproval_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = TenantApproval.searchVarTenantApproval(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListTenantApproval_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = TenantApproval.searchVarTenantApproval(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListTenantApproval_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListTenantApproval_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listTenantApproval(JsonArray l) {
    Optional.ofNullable(searchListTenantApproval_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListTenantApproval_ == null ? 0 : searchListTenantApproval_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<TenantApproval> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("approvalId")).orElse(null) != null)
      w.o(searchListTenantApproval_.get(0));
  }
  ...
  protected void _classAllName(Wrap<String> w) {
    w.o("all tenant approvals");
  }

  @Override
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/approval/tenant");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/approval/tenant");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("Individual tenant approvals per request and per approver. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/approval/tenant-999.png");
  }
  ...
  protected void _pageUriTenantApproval(Wrap<String> c) {
      c.o("/en-us/search/approval/tenant");
  }
}
