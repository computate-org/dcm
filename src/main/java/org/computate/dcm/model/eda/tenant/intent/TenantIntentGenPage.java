 ...
 * Generated: true
 **/
public class TenantIntentGenPage extends TenantIntentGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListTenantIntent_(Wrap<SearchList<TenantIntent>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListTenantIntent_ != null)
      w.o(Optional.ofNullable(searchListTenantIntent_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListTenantIntent_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListTenantIntent_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListTenantIntent_.getStart().longValue();
    Long rows = searchListTenantIntent_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListTenantIntent_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListTenantIntent_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    TenantIntent.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(TenantIntent.displayNameTenantIntent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(TenantIntent.classSimpleNameTenantIntent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListTenantIntent_.getRequest().getQuery()).filter(fq -> fq.startsWith(TenantIntent.varIndexedTenantIntent(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : TenantIntent.varsFqForClass()) {
      String varIndexed = TenantIntent.varIndexedTenantIntent(var);
      String varStored = TenantIntent.varStoredTenantIntent(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(TenantIntent.displayNameTenantIntent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(TenantIntent.classSimpleNameTenantIntent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListTenantIntent_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TenantIntent.varIndexedTenantIntent(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = TenantIntent.searchVarTenantIntent(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    TenantIntent.varsRangeForClass().forEach(var -> {
      String varIndexed = TenantIntent.varIndexedTenantIntent(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(TenantIntent.displayNameTenantIntent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(TenantIntent.classSimpleNameTenantIntent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListTenantIntent_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TenantIntent.varIndexedTenantIntent(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListTenantIntent_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListTenantIntent_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListTenantIntent_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListTenantIntent_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListTenantIntent_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = TenantIntent.searchVarTenantIntent(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", TenantIntent.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListTenantIntent_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = TenantIntent.searchVarTenantIntent(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", TenantIntent.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListTenantIntent_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListTenantIntent_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListTenantIntent_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListTenantIntent_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return TenantIntent.searchVarTenantIntent(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListTenantIntent_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListTenantIntent_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListTenantIntent_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListTenantIntent_.getFacetPivotMinCount()).orElse(0));
  }

  @Override
  protected void _defaultSortVars(List<String> l) {
    if(!searchListTenantIntent_.getDefaultSort()) {
      Optional.ofNullable(searchListTenantIntent_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = TenantIntent.searchVarTenantIntent(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListTenantIntent_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = TenantIntent.searchVarTenantIntent(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListTenantIntent_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListTenantIntent_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listTenantIntent(JsonArray l) {
    Optional.ofNullable(searchListTenantIntent_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListTenantIntent_ == null ? 0 : searchListTenantIntent_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<TenantIntent> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("tenantResource")).orElse(null) != null)
      w.o(searchListTenantIntent_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("tenant intents");
    else if(searchListTenantIntent_ == null || resultCount == 0)
  ...
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/intent/tenant");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/intent/tenant");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("Describing the intent for a new Tenant. Tenants are separate organizations sharing the same cloud resources. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/intent/tenant-999.png");
  }
  ...
  protected void _pageUriTenantIntent(Wrap<String> c) {
      c.o("/en-us/search/intent/tenant");
  }
}
