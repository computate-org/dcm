 ...
 * Generated: true
 **/
public class HostCredentialGenPage extends HostCredentialGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListHostCredential_(Wrap<SearchList<HostCredential>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListHostCredential_ != null)
      w.o(Optional.ofNullable(searchListHostCredential_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListHostCredential_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListHostCredential_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListHostCredential_.getStart().longValue();
    Long rows = searchListHostCredential_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListHostCredential_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListHostCredential_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    HostCredential.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(HostCredential.displayNameHostCredential(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(HostCredential.classSimpleNameHostCredential(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListHostCredential_.getRequest().getQuery()).filter(fq -> fq.startsWith(HostCredential.varIndexedHostCredential(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : HostCredential.varsFqForClass()) {
      String varIndexed = HostCredential.varIndexedHostCredential(var);
      String varStored = HostCredential.varStoredHostCredential(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(HostCredential.displayNameHostCredential(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(HostCredential.classSimpleNameHostCredential(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListHostCredential_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(HostCredential.varIndexedHostCredential(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = HostCredential.searchVarHostCredential(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    HostCredential.varsRangeForClass().forEach(var -> {
      String varIndexed = HostCredential.varIndexedHostCredential(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(HostCredential.displayNameHostCredential(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(HostCredential.classSimpleNameHostCredential(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListHostCredential_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(HostCredential.varIndexedHostCredential(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListHostCredential_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListHostCredential_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListHostCredential_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListHostCredential_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListHostCredential_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = HostCredential.searchVarHostCredential(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", HostCredential.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListHostCredential_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = HostCredential.searchVarHostCredential(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", HostCredential.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListHostCredential_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListHostCredential_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListHostCredential_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListHostCredential_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return HostCredential.searchVarHostCredential(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListHostCredential_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListHostCredential_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListHostCredential_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListHostCredential_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListHostCredential_.getDefaultSort()) {
      Optional.ofNullable(searchListHostCredential_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = HostCredential.searchVarHostCredential(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListHostCredential_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = HostCredential.searchVarHostCredential(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListHostCredential_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListHostCredential_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listHostCredential(JsonArray l) {
    Optional.ofNullable(searchListHostCredential_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListHostCredential_ == null ? 0 : searchListHostCredential_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<HostCredential> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("credentialResource")).orElse(null) != null)
      w.o(searchListHostCredential_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("host credentials");
    else if(searchListHostCredential_ == null || resultCount == 0)
      c.o("no host credential found");
    else
      c.o("host credentials");
  }
  ...
  protected void _classAllName(Wrap<String> w) {
    w.o("all host credentials");
  }

  @Override
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/host-credential");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/host-credential");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("A managed host credential. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/host-credential-999.png");
  }

  @Override
  protected void _classIcon(Wrap<String> c) {
      c.o("<i class=\"{{ FONTAWESOME_STYLE }} fa-input-password\"></i>");
  }

  protected void _pageUriHostCredential(Wrap<String> c) {
      c.o("/en-us/search/host-credential");
  }
}
